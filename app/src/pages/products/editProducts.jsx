...
const EditProducts = () => {
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        setProductDetails({
          ...
          productSpecification: product.specifications.join(', '),
          experiencePeriod: product.experiencePeriod?.toString() || '',
          availability: product.availability
        })

        // Convert existing images to the required format
        ...
        if (product.bannerImage) {
          const bannerIndex = existingImages.findIndex(img => img.uri === product.bannerImage)
          setBannerImage(bannerIndex >= 0 ? bannerIndex : null)
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    Object.entries(productDetails).forEach(([key, value]) => {
      if (key !== 'productSpecification') {
        formData.append(key === 'productName' ? 'name' : key, value)
      }
    })

    // Handle specifications
    productDetails.productSpecification.split(',').forEach(spec => {
      formData.append('specifications[]', spec.trim())
    })

    // Handle images
    selectedImages.forEach((image, index) => {
      // Only append new images (those with local URI)
  ...
  // Render functions (same as AddProducts)
  ...
  // Main render (same structure as AddProducts)
  return (
      ...
      <SafeAreaView className='flex-1' backgroundColor='black'>
        <ScrollView>
          ...
          <View className='flex-1 justify-center items-center' backgroundColor='black'>
            ...
            <View className='w-5/6 h-80 flex-1 justify-center align-center' backgroundColor='black'>
              <CustomTextInput
                label='Product Name'
            ...
            {/* Second Section - Quantity and Brand */}
            <View className='w-5/6 h-80 flex-1 justify-center align-center' backgroundColor='black'>
              <CustomTextInput
                label='Quantity'
            ...
            {/* Third Section - Category and Descriptions */}
            ...
            {/* Fourth Section - Experience Period and Availability */}
            <View className='w-5/6 h-80 flex-1 justify-center align-center' backgroundColor='black'>
              <CustomTextInput
